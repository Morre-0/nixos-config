inputs: { pkgs, ... }: {
  ...
  programs.neovim = {
    ...
    plugins = with pkgs.vimPlugins; [
      gruvbox-material lualine-nvim nvim-web-devicons
      (nvim-treesitter.withPlugins (p: [ p.nix p.lua p.bash p.python p.json p.markdown p.yaml ]))
    ];
    extraLuaConfig = ''
      vim.opt.number = true           
      ...
      require('lualine').setup({ options = { theme = 'gruvbox-material', globalstatus = true } })
    '';
  };

  # ОДИНАРНЫЙ, синтаксически верный блок автоматического развертывания
  home.file = {
    ...
    ".config/picom/picom.conf".text = ''
      ...
      shadow-opacity = 0.4;
      shadow-color = "#1e1e1e";
      blur: { method = "dual_kawase"; strength = 6; background = true; }
      opacity-rule = [ "92:class_g = 'Alacritty'", "95:class_g = 'Rofi'" ];
    '';
  };
